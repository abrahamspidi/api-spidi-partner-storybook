...
Method: **POST**
```
{{base_url}}/api/v1/ext/payment-stops/batch
```
...
**Ordenación:** realiza el orden de visualización con el endpoint dedicado `PATCH {{base_url}}/payment-stop/links/reorder` tras completar el batch.
...
Después de ejecutar un batch, puedes verificar el estado actualizado de múltiples Paradas usando `POST {{base_url}}/api/v1/ext/payment-stops/query`.
...
* Para auditoría, consulta `GET {{base_url}}/payment-stop/{stop_id}/links` con filtros como `status=pending`.
