...
```
{{baseUrl}}/api/v1/payment-stops/stp_123/links?status=active&sort=created_at&order=desc&page=1&page_size=20
```

### 3.2 Auditoría completa con paginación

```
{{baseUrl}}/api/v1/payment-stops/stp_123/links?include_history=true&sort=updated_at&order=desc&page=2&page_size=50
```

### 3.3 Cambios desde una fecha (delta)

```
{{baseUrl}}/api/v1/payment-stops/stp_123/links?updated_after=2025-10-01T00:00:00Z
```

### 3.4 Respuesta liviana (solo campos clave)

```
{{baseUrl}}/api/v1/payment-stops/stp_123/links?status=pending&only_fields=session_id,payment_url,status,expires_at
```
